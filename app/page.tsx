  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      <p className="mt-3 max-w-2xl text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">
        ...
        incorrect) sampled at random from a larger pool. Confidence ratings help you spot uncertainty.
      </p>

      <ul className="mt-8 grid gap-4 sm:grid-cols-2" role="list">
        {quizModulesOrdered.map((mod) => (
          <li key={mod.id} className="flex h-full min-h-0">
            <Link
              href={`/quiz/${mod.id}`}
              className="group flex h-full w-full flex-col rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition hover:border-zinc-300 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-950 dark:hover:border-zinc-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400"
            >
              ...
              <span className="mt-auto pt-4 text-sm font-medium text-zinc-900 underline-offset-4 group-hover:underline dark:text-zinc-100">
                Start module →
              </span>
            </Link>
          </li>
